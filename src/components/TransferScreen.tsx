...
const TransferScreen = ({ user }: TransferScreenProps) => {
  ...
  const parseTransferRequest = (text: string) => {
    ...
    const patterns = [
      /(\w+)(?:에게|한테)\s*(\d+(?:만|천)?원?)?\s*(?:보내|송금|전송)/,
      /(\d+(?:만|천)?원?)?\s*(\w+)(?:에게|한테)\s*(?:보내|송금|전송)/
    ];
    ...
    setTimeout(() => {
      if (transferData) {
        const transferMessage: Message = {
          ...
          type: 'transfer',
          content: `${transferData.receiver}님에게 ${transferData.amount.toLocaleString()}원을 송금합니다.`,
          timestamp: new Date(),
          transferData: {
            ...transferData,
            status: 'processing'
        ...
        setTimeout(() => {
          setMessages(prev => prev.map(msg => 
            msg.id === transferMessage.id 
              ? { ...msg, transferData: { ...msg.transferData!, status: 'completed' } }
              : msg
          ));
        }, 3000);
      } else {
        const systemMessage: Message = {
          id: (Date.now() + 1).toString(),
          type: 'system',
          content: '송금 요청을 이해하지 못했습니다. "김민수에게 5만원 보내줘"와 같이 말씀해주세요.',
          timestamp: new Date(),
        };
          ...
          <div className="flex-1 relative">
            ...
            <Button
              ...
              className={`absolute right-1 top-1/2 transform -translate-y-1/2 w-8 h-8 p-0 rounded-full ${
                isListening ? 'bg-red-500 hover:bg-red-600' : 'bg-slate-400 hover:bg-slate-500'
              }`}
            >
